...
class ChatManager:
    ...
    def __init__(self, model: str = None):
        ...
        self.history: List[Dict[str, Any]] = [
            {"role": "system", "content": (
                f"Você é a {self.agent_name}, assistente de IA do usuário {self.user_name}.\n"
                "Você tem ferramentas para: abrir programas, ler arquivos, escrever arquivos, procurar arquivos, "
                "controlar mouse/teclado, executar comandos, lembrar informações do usuário.\n"
                "Responda em português. Use o nome do usuário. Quando o usuário revelar info pessoal, "
                "use remember_user_info para salvar."
            )}
        ]
    ...
    async def get_response(self, user_message: Optional[str] = None, tools: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            # Roteamento dinâmico: Google Antigravity ou LiteLLM (Gemini/Outros)
            if self.model.startswith("google-antigravity/"):
                from shared.core.antigravity_provider import AntigravityProvider
                ...
                # Determinar api_base baseado no modelo
